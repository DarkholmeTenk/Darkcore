...
public abstract class AbstractTileEntity extends TileEntity
{
	...
	private static int multiBlockInterval = 10;
	...
	private static void refreshConfigs()
	{
		...
		updateInterval = cf.getConfigItem(new ConfigItem("update interval",CType.INT,5,
				"The minimum number of ticks between a single tile entity sending updates",
				"Default: 5")).getInt();
		updateCounterMax = cf.getConfigItem(new ConfigItem("update counter max",CType.INT,20,
				"The maximum number of times a single tile entity can send updates in a short time period",
				"Default: 20")).getInt();
		multiBlockInterval = cf.getConfigItem(new ConfigItem("multi block interval",CType.INT,20,
				"How often a multi block core checks for a valid multiblock structure",
				"Default: 20")).getInt();
	}
